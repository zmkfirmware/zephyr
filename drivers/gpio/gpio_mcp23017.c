...
static int read_port_regs(const struct device *dev, uint8_t reg,
			  uint16_t *buf)
{
	const struct mcp23017_config *const config =
		dev->config;
	...
	uint8_t addr = config->slave | MCP23017_READBIT;

	ret = i2c_write_read(drv_data->i2c, addr,
			&reg, sizeof(reg),
			&port_data, sizeof(port_data));
	if (ret) {
	...
	*buf = sys_le16_to_cpu(port_data);
	...
	LOG_DBG("MCP23017: Read: REG[0x%X] = 0x%X, REG[0x%X] = 0x%X",
			   ...
			   uint16_t value)
{
	...
	LOG_DBG("MCP23017: Write: REG[0x%X] = 0x%X, REG[0x%X] = 0x%X",
		reg, (value & 0xFF), (reg + 1), (value >> 8));

	port_data = sys_cpu_to_le16(value);

	uint8_t buffer_tx[3] = { reg, port_data & 0xFF, port_data >> 8 };

	ret = i2c_write(drv_data->i2c, buffer_tx, 3, config->slave);
	if (ret) {
...
static int mcp23017_config(const struct device *dev,
...
static int mcp23017_port_set_masked_raw(const struct device *dev,
...
static int mcp23017_port_clear_bits_raw(const struct device *dev,
					uint32_t mask)
{
...
static int mcp23017_pin_interrupt_configure(const struct device *dev,
					    gpio_pin_t pin,
...
#define MCP23017_INIT(inst)						\
	...
	DEVICE_AND_API_INIT(mcp23017_##inst, DT_INST_LABEL(inst),	\
			    mcp23017_init, &mcp23017_##inst##_drvdata,	\
			    &mcp23017_##inst##_config,			\
			    POST_KERNEL,				\
